    ...
    for file in all_files:
        with zipfile.ZipFile(file, 'r') as zip_ref:
            ...
            for zip_file in tqdm(zip_file_list):
                ...
                idx = 0
                sentence_id = 0
                data = {'idx': idx, 'dialogue': []}
                with zip_ref.open(zip_file) as zip_data:
                    df = pd.read_excel(zip_data)
                    if 'SENTENCEID' in df.columns:
                        for _, row in tqdm(df.iterrows()):
                            try:
                                row = row.where(pd.notna(row), None)
                                row_data = row.to_dict()
                                if row_data['SENTENCEID'] > sentence_id:
                                    sentence_id = row_data['SENTENCEID']
                                    data['dialogue'].append(row_data)
                                else:
                                    f.write(json.dumps(data, ensure_ascii=False)+'\n')
                                    idx += 1
                                    sentence_id = 0
                                    data = {'idx': idx, 'dialogue': [row_data]}
                            except:
                                # print(row_data)
                                continue
                    else:
