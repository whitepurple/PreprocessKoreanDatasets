...
def main():
    ...
    for file_path in raw_dir.glob('**/*'):
        if file_path.is_file() and file_path.stem not in ['sample', 'preprocess'] and 'preprocessed' not in str(file_path.absolute()):
            ...
            for zip_file in tqdm(zip_file_list):
                if 'json' in zip_file:
                    with zip_ref.open(zip_file) as zip_data:
                        data = zip_data.read()
                        if data:
                            raw_data = json.loads(data)['dataset']['conversations']
                            for rd in raw_data:
