    ...
    for file in all_files:
        ...
        with zipfile.ZipFile(file, 'r') as zip_ref:
            ...
            for zip_file in tqdm(zip_file_list):
                if zip_file.split('.')[-1] == 'json':
                    if 'Validation' in file.absolute().__str__():
                        new_file = (valid_newdatadir/'.'.join(zip_file.split('/')[-1].split('.')[:-1]+['jsonl']))
                    else:
                        new_file = (train_newdatadir/'.'.join(zip_file.split('/')[-1].split('.')[:-1]+['jsonl']))
                    print(new_file)
                    f = new_file.open('w')
